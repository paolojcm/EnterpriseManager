...
namespace EnterpriseManager.Persistence.Specific.State.Repositories
{
	public class StateInfrSpecRepo : IStateDomaSpecRepo
	{
		...
		{
			List<StateDomaSpecEnti>? citiesDomaSpecEnti = null;
			...
			try
			{
				IEnumerable<StateInfrSpecMode> citiesInfrSpecMode = await _iDatabaseUtilitiesSpecServ.GetObjectsAsync<StateInfrSpecMode>(null, sqlStatement, parametersWithTheirValues);
				if ((citiesInfrSpecMode != null) && (citiesInfrSpecMode.Count() > 0))
				{
					citiesDomaSpecEnti = new List<StateDomaSpecEnti>();
					StateDomaSpecEnti? stateDomaSpecEnti = null;
					foreach (StateInfrSpecMode stateInfrSpecMode in citiesInfrSpecMode)
					{
						stateDomaSpecEnti = StateInfrSpecMapp.MapToDomainEntity(stateInfrSpecMode);
						citiesDomaSpecEnti.Add(stateDomaSpecEnti);
					}
				}
			}
			catch (InfrastructureLayerException)
